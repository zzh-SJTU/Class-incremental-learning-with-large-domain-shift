...
def get_loaders(datasets, num_tasks, batch_size, num_workers=4, pin_memory=False, validation=.1):
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        trn_dset, val_dset, tst_dset, curtaskcla = get_datasets(cur_dataset, dc['path'], num_tasks,
...
def get_datasets(dataset, path, num_tasks, validation, trn_transform, tst_transform, class_order=None):
    ...
    if 'mnist' in dataset:
        ...
        for i in range(1, 11):
            with open(os.path.join(path, 'train_data_batch_{}'.format(i)), 'rb') as f:
                d = pickle.load(f)
            x_trn.append(d['data'])
            y_trn.append(np.array(d['labels']) - 1)  
        with open(os.path.join(path, 'val_data'), 'rb') as f:
            d = pickle.load(f)
        x_trn.append(d['data'])
        y_tst = np.array(d['labels']) - 1 
        ...
        all_data, taskcla, class_indices = basedat.get_data(path, num_tasks=num_tasks,
